...
class CharacterModelTest(TestCase):
    ...
    def test_attributes_data_type(self):
        ...
        self.assertTrue(nick_name_received == nick_name_expected, msg="Verify if the nick_name is a string")

        level_received= type(self.character.level)
        level_expected = int

        self.assertTrue(level_received == level_expected, msg="Verify if the level is a integer")
        self.assertTrue(self.character.level > 0, msg="Verify if the level is a integer greater than zero")

        health_received= type(self.character.health)
        health_expected = int

        self.assertTrue(health_received == health_expected, msg="Verify if the health is a integer")
        self.assertTrue(self.character.level > 0, msg="Verify if the health is a integer greater than zero")

        category_name_received= type(self.character.category_name)
        category_name_expected = str

        self.assertTrue(category_name_received == category_name_expected, msg="Verify if the category_name is a string")
